...
when the cohort migrates to composed-Runnables and deletes the file.
...
def test_legacy_chain_file_deleted_DEBT_LOCKED() -> None:
    ...
    assert not LEGACY_CHAIN_FILE.exists(), (
        ...
        f"still exists. Migrate any LLMChain references to composed Runnables, "
        f"delete the file, then update docs/debt-lockfile.yml (flip locked: "
        f"true -> false) and apply the debt-touch-approved label."
    )
...
def test_no_llmchain_references_in_app_DEBT_LOCKED() -> None:
    ...
    assert offenders == [], (
        ...
        f"Migrate to composed Runnables (prompt | llm | parser) and remove "
        f"LLMChain imports, then update docs/debt-lockfile.yml + apply label."
    )
